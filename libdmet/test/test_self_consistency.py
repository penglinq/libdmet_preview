...
def test_self_consistency(orth, kind):
    """
    ...
    C_ao_iao, C_ao_iao_val, C_ao_iao_virt, lo_labels = \
            make_basis.get_C_ao_lo_iao(Lat, kmf, minao='minao',
                                       full_return=True,
                                       return_labels=True)
    ...
    for iter in range(MaxIter):
        ...
        rho, Mu, res = dmet.RHartreeFock(Lat, vcor, Filling, Mu, beta=beta,
                                         ires=True, symm=True, labels=lo_labels)
        Lat.update_Ham(rho*2.0)
        
        log.section("\nconstructing impurity problem\n")
        ImpHam, H1e, basis = dmet.ConstructImpHam(Lat, rho, vcor, matching=True,
                                                  int_bath=int_bath, orth=orth,
                                                  incore=True, kind=kind)
...
if __name__ == "__main__":
    test_self_consistency(orth=True, kind='svd')
